...
describe("AbstractSaga", () => {
  ...
  describe("rollback", () => {
    ...
    it("rolls back steps in reverse order", async () => {
      saga.addStep(new NumberedStep("1")).addStep(new NumberedStep("2")).addStep(new ThrowingStep());
      const result = await saga.orchestrate();
    ...
    it("steps can mark themselves so they are also rolled back", async () => {
      saga.addStep(new NumberedStep("1")).addStep(new NumberedStep("2")).addStep(new ThrowingStep(true));
      const result = await saga.orchestrate();
